#!/usr/bin/env python3
"""Install a host directory tree into a RADix ext filesystem image with debugfs."""
...
def main() -> int:
    ...
    if not tree.exists():
        return 0
    if not tree.is_dir():
        print(f"RADix package rootfs payload is not a directory: {tree}", file=sys.stderr)
        return 2
